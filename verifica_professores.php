...
if(isset($_GET['cod_aluno_dia_aula'])  && ($_GET['cod_aluno_dia_aula'] <> ''))
{
	mysql_select_db($database_conecta, $conecta);
	$query_remarcado = "SELECT aa.*, ap.*, pp.cod_professor
						FROM aluno_dia_aula aa, aluno_pacote_aula ap, aluno_pacote_professor pp 
						WHERE aa.cod_aluno_pacote_aula = ap.cod_aluno_pacote_aula 
						AND aa.cod_aluno_dia_aula = pp.cod_aluno_dia_aula
						AND aa.cod_aluno_dia_aula = '".$_GET['cod_aluno_dia_aula']."'";
...
<table width="100%">
  ...
  <tr valign="baseline">
    ...
    <td width="82%"><select name="cod_professor" id="cod_professor">
      <?php
        do { 
        ?>
      <option value="<?php echo $row_verifica['cod_professor']; ?>" <?php echo($row_remarcado['cod_professor'] == $row_verifica['cod_professor'])? 'selected="selected"':""; ?>><?php echo $row_verifica['nome_professor']?></option>
      <?php
  ...
  <tr valign="baseline">
    ...
    <td><input name="hr_inicio" id="hr_inicio" type="text" class="hora hr_inicio" value="" size="5" maxlength="5" />
      Hora final:
      <input name="hr_fim" id="hr_fim" type="text" class="hora hr_fim" value="" size="5" maxlength="5" />
...
function calcular(){
	...
	var entrada 	= (parseInt(document.formu.entrada.value));
	var inicio 		= (parseInt(document.formu.inicio.value));
	var termino 	= (parseInt(document.formu.termino.value));
	var saida 		= (parseInt(document.formu.saida.value));	
	
	var total = ( (inicio - entrada) + (saida - termino) );
	
	alert (total);

}

$(".hr_inicio").change(function(){
	
	var horaInicio = $(this).val();

	var hr_inicio = horaInicio.split(":");
	
	if(hr_inicio[0] >= 23)
	{
		var hora = (parseInt(hr_inicio[0]) + 1);
		if(hora == 24)
		{
			hora = 0;	
		}
	...
	var resultado = hora + ":" + hr_inicio[1];
	var n = resultado.length;
	if(n == 4)
	{
		resultado = "0" + resultado;	
	}
	$(".hr_fim").val(resultado);	
});
...
$(".hora").attr("placeholder", "hh:mm");
	...
	$(".dia_avulso").html(' <input name="dia2" value="<?php echo voltadobanco($row_remarcado['dia']); ?>" class="dt1 data" size="10" maxlength="10" /> <br><br> <strong>Observação</strong><br><br><textarea name="informacao" id="informacao" cols="45" rows="5"></textarea>').promise().done(function(){
	...
	$("#dia").change(function(e){
		var dia = $(this).val();
		if(dia == 10){
			$(".dia_avulso").html('Selecione um dia: <input name="dia2" value="<?php echo date('d/m/Y'); ?>" class="dt1 data" size="10" maxlength="10" />').promise().done(function(){
				$(".data").mask('99/99/9999');
				$('.data.dt1').datepicker({
					minDate: new Date()
				});
