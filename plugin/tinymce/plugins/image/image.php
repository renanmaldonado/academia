	...
	$("#get-recent").bind("click", function () {
		...
		$.getJSON('recent.php',{}, function(returned){ 
			if(returned.success == 1){
				...
				$('#recent-images').empty();
				$('#recent-images').append('<center>Não há imagens recentes encontrada.</center>');
			}
		});
	});
	...
	$(document).on('click', 'button#lib-back', function () {
		...
		$.getJSON('lib.php' + '?dummy=' + new Date().getTime(),{path: $(this).attr("rel")}, function(returned){ 
			if(returned.success == 1){
				...
				$('#gallery-images').empty();
				$('#gallery-images').append('<center>Nenhuma imagem foi encontrada na pasta.</center>');
			}
		});
				...
				<div class="span12" style="margin-top: 20px;">
					...
					<div class="tabbable tabs-left">
						<ul class="nav nav-tabs" id="myTab">
							<li><a href="#tab1" data-toggle="tab"><i class="icon-globe"></i> Inserir url</a></li>
							<?php if(CanAcessLibrary()){?>
							<li><a href="#tab2" data-toggle="tab" id="get-lib"><i class="icon-folder-open"></i> Biblioteca</a></li>
							<?php }?>
							<?php if(CanAcessUploadForm()){?>
							<li><a href="#tab3" data-toggle="tab"><i class="icon-upload"></i> Enviar arquivo</a></li>
							<?php }?>
							<li><a href="#tab4" data-toggle="tab" id="get-recent"><i class="icon-time"></i> Recente</a></li>
						</ul>
...
<p>
<input type="text" id="title" name="title" value="<?php echo $title;?>" placeholder="Título" title="Title">
</p>

<p>
<input type="text" id="alt" name="alt" value="<?php echo $alt;?>" placeholder="Descrição" title="Description">
</p>
<br/>
<p>
<input type="text" id="width" name="width" class="input-small dimensions" placeholder="Largura" title="Width" value="<?php echo $width;?>"> &times; <input type="text" id="height" name="height" class="input-small dimensions" placeholder="Altura" title="Height" value="<?php echo $height;?>"> <br/><input type="checkbox" id="constrain" name="constrain" checked="checked"> Forçar proporção original
</p>
<br/>
<p>
<select id="float" name="float">
<option value="">Alinhamento: None</option>
<option value="left" <?php echo ($align == 'left' ? 'selected="selected"' : '');?>>Esquerda</option>
<option value="right" <?php echo ($align == 'right' ? 'selected="selected"' : '');?>>Direita</option>
</select>
</p>
...
<br/>
<p>
<input type="checkbox" id="do_link" name="do_link"> Digite um link para a imagem
</p>
...
<option value="_self">Trageto: none</option>
						...
						<div style="clear: both;"></div>
							...
							<div class="tab-pane" id="tab2">
								...
								<div class="pull-left" style="padding-left: 11px;">
									<button class="btn" disabled id="lib-back" rel="<?php echo LIBRARY_FOLDER_PATH; ?>"><i class="icon-hand-left"></i> Voltar</button>&nbsp;&nbsp;&nbsp;<a href="" title="atualizar" rel="<?php echo LIBRARY_FOLDER_PATH; ?>" id="refresh"><i class="icon-refresh"></i></a>
					
								</div>
								
								<div class="pull-right" style="padding-right: 12px;">
									<input type="text" class="input-medium" id="search" placeholder="Pesquisar">
								</div>
								<?php if(CanCreateFolders()){?>
								<div class="pull-right" style="padding-right: 12px;">
									<span id="new-folder-msg"></span>
									<div class="input-append">
										<input class="input-medium" id="newfolder_name" type="text" placeholder="Criar pasta">
								...
								<p style="padding-right: 20px;" class="pull-right transparent"><a id="toggle-layout" href="" title="Alternar Lista / Grade Visualizações"><i class="icon-th-list"></i></a></p>
								<div style="clear: both;"></div>
								</div>
...
<div>
<div class="pull-left">
	<p class="muted">Tamanho máximo do arquivo: <?php echo $upload_mb;?> MB</p>
</div>
...
</select>&nbsp;&nbsp;&nbsp;<a href="" title="Atualizar lista da pasta" id="refresh-dirs"><i class="icon-refresh"></i></a>
</p>
</div>
...
<form id="upload" method="post" action="upload.php" enctype="multipart/form-data">
			...
			<div id="drop">
				...
				<a class="btn">Clique para selecionar</a>
